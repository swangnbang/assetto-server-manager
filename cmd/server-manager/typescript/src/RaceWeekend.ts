...
export namespace RaceWeekend {
  ...
  export class EditSession {
    ...
    private initSessionTypeSwitch(): void {
      ...
      $sessionSwitcher.on("change", (e: ChangeEvent) => {
        ...
        this.$raceWeekendSession
          .find(".sessions .tab-pane")
  ...
  export class View {
    ...
    private openManageEntryListModal(e: JQuery.ClickEvent): void {
      ...
      const sessionID = $(e.currentTarget)
        .closest(".race-weekend-session")
        .attr("id") as string;
  ...
  abstract class PreviewModal {
    ...
    protected buildTableDataForEntrant(
      entrant: SessionPreviewEntrant,
      pos?: number,
    ): JQuery<HTMLTableDataCellElement> {
    ...
    protected buildClassKey(classes: Map<string, string>) {
      ...
      if (Object.entries(classes).length > 1) {
        for (const [className, classColor] of Object.entries(classes)) {
          ...
          let $colorText = $("<div>")
            .attr({ class: "class-key__name" })
            .text(className);

          $tableKey.append(
            $("<div>")
              .attr({ class: "class-key" })
              .append($colorBlock, $colorText),
          );
        }
      }
    }
  ...
  class SessionTransition extends PreviewModal {
    ...
    protected updateValues(): void {
      this.resultStart = parseInt(
        this.$elem.find("#ResultsStart").val() as string,
      );
      this.resultEnd = parseInt(this.$elem.find("#ResultsEnd").val() as string);
      this.reverseGrid = parseInt(
        this.$elem.find("#ReverseGrid").val() as string,
      );
      this.gridStart = parseInt(this.$elem.find("#GridStart").val() as string);
      this.sortType = this.$elem.find("#ResultsSort").val() as string;
      this.availableResultsForSorting = this.$elem
        .find("#AvailableResults")
        .val() as string[];
      this.startOnFastestLapTyre = this.$elem
        .find("#ForceUseTyreFromFastestLap")
        .is(":checked");

      if (
        this.sortType == "fastest_multi_results_lap" ||
        this.sortType == "number_multi_results_lap"
      ) {
      ...
      $.ajax(
        `/race-weekend/${RaceWeekendID}/grid-preview?parentSessionID=${this.parentSessionID}&childSessionID=${this.childSessionID}`,
        {
    ...
    protected saveValues(): void {
      $.ajax(
        `/race-weekend/${RaceWeekendID}/update-grid?parentSessionID=${this.parentSessionID}&childSessionID=${this.childSessionID}`,
        {
          data: this.packageValues(),
  ...
  class EntryListPreview extends PreviewModal {
    ...
    protected updateValues(): void {
      ...
      $.ajax(
        `/race-weekend/${RaceWeekendID}/entrylist-preview?sessionID=${this.sessionID}&sortType=${this.sortType}&reverseGrid=${this.reverseGrid}`,
        {
          type: "GET",
        },
      ).then((response: GridPreview) => {
        ...
        for (let i = 0; i < grid.length; i++) {
          ...
          if (grid[i].Class) {
            $pos.css({
              "background-color": grid[i].ClassColor,
              color: "white",
            });
          }
    ...
    protected saveValues(): void {
      $.ajax(
        `/race-weekend/${RaceWeekendID}/update-entrylist?sessionID=${this.sessionID}&sortType=${this.sortType}&reverseGrid=${this.reverseGrid}`,
        {
          type: "GET",
        },
      ).then(() => {
