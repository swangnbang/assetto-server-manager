...
export class RaceControl {
  ...
  private readonly liveTimings: LiveTimings = new LiveTimings(
    this,
    this.liveMap,
  );
  ...
  private handleWebsocketMessage(ev: MessageEvent): void {
    ...
    switch (message.EventType) {
      ...
      case EventRaceControl:
        ...
        if (
          this.status.SessionInfo.Track !== this.track ||
          this.status.SessionInfo.TrackConfig !== this.trackLayout
        ) {
        ...
        this.$eventTitle.text(
          RaceControl.getSessionType(this.status.SessionInfo.Type) +
            " at " +
            this.status.TrackInfo!.name,
        );
        $("#track-location").text(
          this.status.TrackInfo.city + ", " + this.status.TrackInfo.country,
        );
      ...
      case EventChat:
        ...
        chatMessageSender
          .attr(
            "style",
            "color: " + randomColorForDriver(message.Message.DriverGUID),
          )
          .text(
            hoursString +
              ":" +
  ...
  private showEventCompletion() {
    ...
    if (this.status.SessionInfo.Time > 0) {
      let timeInMS =
        this.status.SessionInfo.Time * 60 * 1000 +
        (this.status.SessionInfo.WaitTime / 126.166667) * 1000 -
        moment
          .duration(
            moment().utc().diff(moment(this.status.SessionStartTime).utc()),
          )
          .asMilliseconds();

      let days = Math.floor(timeInMS / 8.64e7);
      ...
      if (
        this.status.ConnectedDrivers &&
        this.status.ConnectedDrivers.GUIDsInPositionalOrder.length > 0
      ) {
        let driver =
          this.status.ConnectedDrivers.Drivers[
            this.status.ConnectedDrivers.GUIDsInPositionalOrder[0]
          ];
      ...
      timeRemaining =
        this.status.SessionInfo.Laps - lapsCompleted + " laps remaining";
    }
  ...
  private buildSessionInfo() {
    ...
    $roadTempWrapper.attr(
      "style",
      "background-color: " +
        getColorForPercentage(this.status.SessionInfo.RoadTemp / 40),
    );
    $roadTempWrapper.attr(
      "data-original-title",
      "Road Temp: " + this.status.SessionInfo.RoadTemp + "°C",
    );
    ...
    $ambientTempWrapper.attr(
      "style",
      "background-color: " +
        getColorForPercentage(this.status.SessionInfo.AmbientTemp / 40),
    );
    $ambientTempWrapper.attr(
      "data-original-title",
      "Ambient Temp: " + this.status.SessionInfo.AmbientTemp + "°C",
    );
  ...
  private showTrackWeatherImage(): void {
    ...
    $.get("/content/weather/" + pathFinal + "/preview.jpg")
      .done(function () {
        // preview for skin exists
        $currentWeather
          .attr("src", "/content/weather/" + pathFinal + "/preview.jpg")
          .show();
      })
      .fail(function () {
    ...
    $currentWeather.attr(
      "alt",
      "Current Weather: " +
        prettifyName(this.status.SessionInfo.WeatherGraphics, false),
    );
  }
  ...
  private getTrackImageURL(): string {
    ...
    return (
      ...
      sessionInfo.Track +
      "/ui" +
      (!!sessionInfo.TrackConfig ? "/" + sessionInfo.TrackConfig : "") +
      "/preview.png"
    );
  }
  ...
  private handleIFrames(): void {
    ...
    $document.on("change", ".live-frame-link", function (e: ChangeEvent) {
      ...
      if (value) {
        let $liveTimingFrame = $this
          .closest(".live-frame-wrapper")
          .find(".live-frame");
        $this
          .closest(".live-frame-wrapper")
    ...
    $document.find("#add-live-frame").click(function () {
      ...
      $copy
        .find(".embed-responsive")
        .attr("class", "d-none embed-responsive embed-responsive-16by9");
...
class LiveMap implements WebsocketHandler {
  ...
  // live map track dots
  private dots: Map<string, JQuery<HTMLElement>> = new Map<
    string,
    JQuery<HTMLElement>
  >();
  ...
  public handleWebsocketMessage(message: WSMessage): void {
    switch (message.EventType) {
      case EventRaceControl:
        ...
        for (const connectedGUID in this.raceControl.status.ConnectedDrivers!
          .Drivers) {
          const driver =
            this.raceControl.status.ConnectedDrivers!.Drivers[connectedGUID];
          ...
          if (!this.dots.has(driver.CarInfo.DriverGUID)) {
            ...
            let $driverDot = this.buildDriverDot(
              driver.CarInfo,
              driver.LastPos as CarUpdateVec,
            ).show();
        ...
        $(".dot").css({
          transition:
            this.raceControl.status.CurrentRealtimePosInterval + "ms linear",
        });
        break;
      ...
      case EventNewConnection:
        ...
        this.dots.set(
          connectedDriver.DriverGUID,
          this.buildDriverDot(connectedDriver),
        );
      ...
      case EventCarUpdate:
        ...
        if (
          !this.raceControl.status!.CarIDToGUID.hasOwnProperty(update.CarID)
        ) {
          return;
        }
        ...
        let speed = Math.floor(
          Math.sqrt(
            Math.pow(update.Velocity.X, 2) + Math.pow(update.Velocity.Z, 2),
          ) * 3.6,
        );
        let speedUnits = "Km/h ";
      ...
      case EventCollisionWithEnv:
        ...
        let collisionMapPoint = this.translateToTrackCoordinate(
          collisionData.WorldPos,
        );
  ...
  private translateToTrackCoordinate(vec: CarUpdateVec): CarUpdateVec {
    ...
    out.X =
      ((vec.X + this.trackXOffset + this.trackMargin) / this.trackScale) *
      this.mapScaleMultiplier;
    out.Z =
      ((vec.Z + this.trackZOffset + this.trackMargin) / this.trackScale) *
      this.mapScaleMultiplier;

    return out;
  }

  private buildDriverDot(
    driverData: SessionCarInfo,
    lastPos?: CarUpdateVec,
  ): JQuery<HTMLElement> {
    if (this.dots.has(driverData.DriverGUID)) {
      return this.dots.get(driverData.DriverGUID)!;
    }

    const $driverName = $("<span class='name'/>").text(
      driverData.DriverInitials,
    );
    const $info = $("<span class='info'/>").text("0").hide();

    const $dot = $(
      "<div class='dot' style='background: " +
        randomColorForDriver(driverData.DriverGUID) +
        "'/>",
    )
  ...
  private correctMapDimensions(): void {
    ...
    if (
      this.$trackMapImage.height()! / this.$trackMapImage.width()! >
      LiveMap.mapRotationRatio
    ) {
      // rotate the map
      ...
      this.mapScaleMultiplier =
        this.$trackMapImage.width()! /
        this.raceControl.status.TrackMapData.width;

      this.$map.closest(".map-container").css({
        "max-height":
          this.raceControl.status.TrackMapData.width * this.mapScaleMultiplier +
          20,
      });
...
class LiveTimings implements WebsocketHandler {
  ...
  constructor(raceControl: RaceControl, liveMap: LiveMap) {
    ...
    $(document).on(
      "submit",
      "#broadcast-chat-form",
      this.processChatForm.bind(this),
    );
    $(document).on(
      "submit",
      "#admin-command-form",
      this.processAdminCommandForm.bind(this),
    );
    $(document).on(
      "submit",
      "#kick-user-form",
      this.processKickUserForm.bind(this),
    );
    $(document).on(
      "submit",
      "#send-chat-form",
      this.processSendChatForm.bind(this),
    );
  }
  ...
  public handleWebsocketMessage(message: WSMessage): void {
    if (message.EventType === EventRaceControl) {
      ...
      if (this.raceControl.status.ConnectedDrivers) {
        const driver =
          this.raceControl.status.ConnectedDrivers.Drivers[
            closedConnection.DriverGUID
          ];

        if (
          driver &&
          (driver.LoadedTime.toString() === "0001-01-01T00:00:00Z" ||
            !driver.TotalNumLaps)
        ) {
          // a driver joined but never loaded, or hasn't completed any laps. remove them from the connected drivers table.
          this.$connectedDriversTable
            .find("tr[data-guid='" + closedConnection.DriverGUID + "']")
            .remove();
  ...
  private populateConnectedDrivers(): void {
    ...
    for (const driverGUID of this.raceControl.status.ConnectedDrivers
      .GUIDsInPositionalOrder) {
      const driver =
        this.raceControl.status.ConnectedDrivers.Drivers[driverGUID];
  ...
  private populateDisconnectedDrivers(): void {
    if (
      !this.raceControl.status ||
      !this.raceControl.status.DisconnectedDrivers
    ) {
      return;
    }

    for (const driverGUID of this.raceControl.status.DisconnectedDrivers
      .GUIDsInPositionalOrder) {
      const driver =
        this.raceControl.status.DisconnectedDrivers.Drivers[driverGUID];
  ...
  private newRowForDriver(
    driver: Driver,
    addingToConnectedTable: boolean,
  ): JQuery<HTMLElement> {
    const $tr = $(
      addingToConnectedTable
    ...
    if (addingToConnectedTable) {
      // driver dot
      const driverDot = this.liveMap.getDotForDriverGUID(
        driver.CarInfo.DriverGUID,
      );
  ...
  private addDriverToTable(
    ...
    if (!$tr.length) {
      addTrToTable = true;
      $tr = this.newRowForDriver(
        driver,
        addingDriverToConnectedTable,
    ...
    if (addingDriverToConnectedTable) {
      $tr
        .find(".driver-pos")
        .text(
          driver.Position === 255 || driver.Position === 0
            ? ""
            : driver.Position,
        );
    }
    ...
    $tr
      ...
      .text(
        carInfo.CarName
          ? carInfo.CarName
          : prettifyName(driver.CarInfo.CarModel, true),
      );
    ...
    if (addingDriverToConnectedTable) {
      ...
      if (
        moment(carInfo.LastLapCompletedTime)
          .utc()
          .isAfter(moment(this.raceControl.status!.SessionStartTime).utc())
      ) {
        // only show current lap time text if the last lap completed time is after session start.
        currentLapTimeText = msToTime(
          moment().utc().diff(moment(carInfo.LastLapCompletedTime).utc()),
          false,
        );
      }
    ...
    $tr
      .find(".top-speed")
      .text(topSpeed ? topSpeed.toFixed(2) + speedUnits : "");
    ...
    if (addingDriverToConnectedTable) {
      ...
      if (driver.Collisions) {
        for (const collision of driver.Collisions) {
          const collisionID =
            driver.CarInfo.DriverGUID + "-collision-" + collision.ID;

          if (
            moment(collision.Time)
              .utc()
              .add("10", "seconds")
              .isSameOrAfter(moment().utc()) &&
            !$("#" + collisionID).length
            ...
            if (collision.Type === Collision.WithCar) {
              $tag.text(
                "Crash with " +
                  collision.OtherDriverName +
                  " at " +
                  crashSpeed.toFixed(2) +
                  speedUnits,
              );
            } else {
              $tag.text(
    ...
    if (
      !addingDriverToConnectedTable &&
      (!carInfo.NumLaps || carInfo.NumLaps === 0)
    ) {
      return;
    }
